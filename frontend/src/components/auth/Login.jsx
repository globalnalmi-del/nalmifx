  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        if (res.data.data.user.emailVerified === false) {
    ...
    <div className="min-h-screen flex">
      ...
      <div className="w-full lg:w-1/2 flex flex-col justify-center px-8 lg:px-16 xl:px-24" style={{ backgroundColor: '#000000' }}>
        <div className="max-w-md w-full mx-auto">
          ...
          {showVerificationBanner && (
            <div className="mb-4 p-4 rounded-xl bg-yellow-500/10 border border-yellow-500/20">
              <div className="flex items-start gap-3">
                <Mail size={20} className="text-yellow-400 mt-0.5" />
                <div className="flex-1">
                  ...
                  {verificationMessage && (
                    <p className="text-green-400 text-xs mt-2">{verificationMessage}</p>
                  )}
                  <button
                    onClick={handleResendVerification}
                    disabled={resendingVerification}
                    className="mt-2 text-xs text-yellow-400 underline hover:text-yellow-300 disabled:opacity-50"
                  >
          ...
          {/* Demo Login Button - Only for local development */}
          <button
            type="button"
            onClick={handleDemoLogin}
            disabled={demoLoading}
            className="w-full py-4 rounded-xl font-semibold text-white transition-all disabled:opacity-50 mt-4 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-500 hover:to-emerald-500"
          >
            {demoLoading ? (
              <span className="flex items-center justify-center gap-2">
                <Loader2 className="animate-spin" size={20} />
