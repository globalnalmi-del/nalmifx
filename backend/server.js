...
app.get('/api/health', (req, res) => {
  res.json({
    success: true,
    message: 'NalmiFX Trading API is running',
...
server.listen(PORT, () => {
  ...
  // Auto-seed email templates if none exist
  const seedEmailTemplates = async () => {
    try {
      const count = await EmailTemplate.countDocuments();
      if (count === 0) {
        await EmailTemplate.insertMany(adminEmailTemplatesRoutes.defaultTemplates);
